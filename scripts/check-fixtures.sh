...
require_contains "$ROOT/README.md" '1 个封面页 \+ 2-7 个内容页' "README default page count"
require_contains "$ROOT/README.md" '不会默认生成 `金句页`' "README quote page default"
...
require_contains "$CONTRIBUTING_FILE" 'one cover page plus 2-7 content pages' "contributing default page count"
require_contains "$CONTRIBUTING_FILE" '`金句页` is not generated by default' "contributing quote page default"
...
require_contains "$ROOT/references/xhs-page-types.md" '默认不要为了凑页数自动加入金句页' "quote page default guardrail"
require_contains "$ROOT/references/xhs-page-types.md" '不要默认把最后一页改成金句页' "last page quote guardrail"
require_contains "$ROOT/references/xhs-page-types.md" '6-8 页：封面页 \+ 5-7 个内容页' "2-7 content page page-count guidance"
require_contains "$ROOT/references/prompt-quality-checklist.md" 'Do not add a `金句页` by default' "quote page checklist guardrail"
...
require_contains "$README_CN_FILE" '1 个封面页 \+ 2-7 个内容页' "Chinese README default page count"
require_contains "$README_CN_FILE" '不会默认生成 `金句页`' "Chinese README quote page default"
...
require_contains "$README_EN_FILE" '1 cover card \+ 2-7 content cards' "English README default page count"
require_contains "$README_EN_FILE" 'does not add a quote page by default' "English README quote page default"
...
require_contains "$SKILL_FILE" 'long article without page structure, create one cover claim and 2-7 content pages' "unstructured input normalization rule"
...
while IFS='|' read -r fixture expected_pages style article; do
  ...
  if [[ -f "$cards_file" ]]; then
    ...
    require_not_contains "$cards_file" '金句页' "unexpected quote page"
  ...
  if [[ -f "$prompts_file" ]]; then
    ...
    require_not_contains "$prompts_file" '金句页' "unexpected quote page"
