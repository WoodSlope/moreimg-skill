...
- Unpaged long articles generate one cover page plus 2-7 content pages, then use the same final prompt skeleton.
- `金句页` is not generated by default; it appears only when the user explicitly asks for a quote page, closing page, final emphasis, or shareable takeaway.
